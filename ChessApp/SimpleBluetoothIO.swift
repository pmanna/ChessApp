...
class SimpleBluetoothIO: NSObject {
    ...
    init(serviceUUID: String, peripheralName: String = "", delegate: SimpleBluetoothIODelegate?) {
...
extension SimpleBluetoothIO: CBCentralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String : Any], rssi RSSI: NSNumber) {
        if peripheralName.count > 0 && peripheral.name != peripheralName { return }
...
extension SimpleBluetoothIO: CBPeripheralDelegate {
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        guard let characteristics = service.characteristics, let delegate = delegate else {
            return
        }
        ...
        delegate.simpleBluetoothIO(simpleBluetoothIO: self, isReady: true)
    }
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        delegate.simpleBluetoothIO(simpleBluetoothIO: self, didReceiveValue: data)
    }
}
