...
int M = 136, S = 128, I = 8e3, C = 799, Q, O, K, N, k = 8;
...
void inputPlayerMove(const char *humanMove)
{
    strncpy(c, humanMove, 4);
    c[4] = '\0';

    K = *c - 16 * c[1] + C;
    L = c[2] - 16 * c[3] + C; /* parse entered move */

    k ^= D(k, -I, I, Q, O, 8, 2) - I ? 0 : 24; /* think or check & do*/
}
...
void getUMaxMove(char *out)
{
    ...
    sprintf(out, "%c%c%c%c", 'a' + (K & 7), '8' - (K >> 4), 'a' + (L & 7), '8' - (L >> 4));
}
